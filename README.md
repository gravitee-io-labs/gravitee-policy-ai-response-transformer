...
3. Build OpenAI-compatible `/chat/completions` request:
   ...
   - `user` message:
     - full response body when targeting is disabled,
     - selected JSON target when targeting is enabled.
4. Apply transformed output:
   - targeting disabled: replace full response body,
   - targeting enabled: apply target mode (`REPLACE_TARGET` or `MERGE_OBJECT_AT_ROOT`).
5. Optional post-processing (`parseLlmResponseJsonInstructions=true`):
   - Parse JSON `{ "status_code": ..., "headers": {...}, "body": ... }`.
...
```json
{
  "prompt": "Rewrite only the nested message field in Spanish.",
  "llmSourceMode": "INLINE",
  "jsonTargetingEnabled": true,
  "targetPath": "$.data.message",
...
```json
{
  "prompt": "Return only { \"newsbrief\": \"...\", \"status\": \"success|failed|upcoming\" }.",
  "llmSourceMode": "INLINE",
  "jsonTargetingEnabled": true,
  "targetPath": "$",
